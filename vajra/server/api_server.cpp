...
#include "api/python_rule.h"
#include "api/rule_builder.h"
...
int main()
{
    ...
    router["/event/event_process_tree"].post([connection_pool](const HTTPMessage& req) -> HTTPMessage
                             {
                                 auto conn = connection_pool->GetConnection();
                                 auto response = event_process_tree(req, conn);
                                 connection_pool->ReturnConnection(conn);
                                 return response;
                             });
    ...
    router["/node/node_log_list"].get([connection_pool](const HTTPMessage& req) -> HTTPMessage
                              {
                                  auto conn = connection_pool->GetConnection();
                                  auto response = node_log_list(req, conn);
                                  connection_pool->ReturnConnection(conn);
                                  return response;
                              });
    ...
    router["/python_rule/rule_add"].post([connection_pool](const HTTPMessage& req) -> HTTPMessage
                              {
                                  auto conn = connection_pool->GetConnection();
                                  auto response = rule_add(req, conn);
                                  connection_pool->ReturnConnection(conn);
                                  return response;
                              });
    router["/python_rule/rule_list"].get([connection_pool](const HTTPMessage& req) -> HTTPMessage
                              {
                                  auto conn = connection_pool->GetConnection();
                                  auto response = rule_list(req, conn);
                                  connection_pool->ReturnConnection(conn);
                                  return response;
                              });
    router["/python_rule/rule_update"].post([connection_pool](const HTTPMessage& req) -> HTTPMessage
                              {
                                  auto conn = connection_pool->GetConnection();
                                  auto response = rule_update(req, conn);
                                  connection_pool->ReturnConnection(conn);
                                  return response;
                              });
    router["/python_rule/rule_delete"].delete_([connection_pool](const HTTPMessage& req) -> HTTPMessage
                              {
                                  auto conn = connection_pool->GetConnection();
                                  auto response = rule_delete(req, conn);
                                  connection_pool->ReturnConnection(conn);
                                  return response;
                              });
    router["/rule_builder/rule_add"].post([connection_pool](const HTTPMessage& req) -> HTTPMessage
                              {
                                  auto conn = connection_pool->GetConnection();
                                  auto response = rule_builder_rule_add(req, conn);
                                  connection_pool->ReturnConnection(conn);
                                  return response;
                              });
    router["/rule_builder/rule_list"].get([connection_pool](const HTTPMessage& req) -> HTTPMessage
                              {
                                  auto conn = connection_pool->GetConnection();
                                  auto response = rule_builder_rule_list(req, conn);
                                  connection_pool->ReturnConnection(conn);
                                  return response;
                              });
    router["/rule_builder/rule_update"].post([connection_pool](const HTTPMessage& req) -> HTTPMessage
                              {
                                  auto conn = connection_pool->GetConnection();
                                  auto response = rule_builder_rule_update(req, conn);
                                  connection_pool->ReturnConnection(conn);
                                  return response;
                              });
    router["/rule_builder/rule_delete"].delete_([connection_pool](const HTTPMessage& req) -> HTTPMessage
                              {
                                  auto conn = connection_pool->GetConnection();
                                  auto response = rule_builder_rule_delete(req, conn);
                                  connection_pool->ReturnConnection(conn);
                                  return response;
                              });
    router["/python_rule/tables_list"].get([connection_pool](const HTTPMessage& req) -> HTTPMessage
                            {
                                auto conn = connection_pool->GetConnection();
                                auto response = tables_list(req, conn);
                                connection_pool->ReturnConnection(conn);
                                return response;
                            });
    ...
    router["/settings/manual_purge"].get([connection_pool](const HTTPMessage& req) -> HTTPMessage
                                             {
                                                 auto conn = connection_pool->GetConnection();
                                                 auto response = manual_purge(req, conn);
                                                 connection_pool->ReturnConnection(conn);
                                                 return response;
                                             });
    router["/settings/data_retention"].get([connection_pool](const HTTPMessage& req) -> HTTPMessage
                                             {
                                                 auto conn = connection_pool->GetConnection();
                                                 auto response = data_retention(req, conn);
                                                 connection_pool->ReturnConnection(conn);
                                                 return response;
                                             });
    router["/settings/data_retention_days"].get([connection_pool](const HTTPMessage& req) -> HTTPMessage
                                             {
                                                 auto conn = connection_pool->GetConnection();
                                                 auto response = data_retention_days(req, conn);
                                                 connection_pool->ReturnConnection(conn);
                                                 return response;
                                             });
