...
public:
    ...
    virtual int unload() = 0;
    virtual PluginInfo version() = 0;
    virtual std::vector<std::tuple<std::string,std::string,std::string,std::string, RemediationPolicy, std::function<bool(const BlockingEvent)>>> process_blocking_rules() = 0;
};
