...
HTTPMessage os_count(HTTPMessage request, std::shared_ptr<pqxx::connection> connection)
{
    ...
    try
    {
        ...
        connection->prepare("get_os_count", "select os_name, count(*) from nodes group by os_name");
...
HTTPMessage weekly_alerts_count(HTTPMessage request, std::shared_ptr<pqxx::connection> connection)
{
    ...
    int weekday=0;
    std::stringstream ss;
    std::vector<std::string> day={"Sunday","Monday","Tuesday","Wednesday","Thrusday","Friday","Saturday"};
    int unique_os;
    try
    {
        for (const auto &it : request.query)
        ...
        for (int i = 1; i <=7;)
        {
            ...
            alert_count[day[weekday]]["File Alert"]=0;
            alert_count[day[weekday]]["Process Alert"]=0;
            alert_count[day[weekday]]["Socket Alert"]=0;
            for (pqxx::result::const_iterator c = result.begin(); c != result.end(); ++c)
            {
                alert_count[day[weekday]][c["alert_type"].as<std::string>()]=c["count"].as<int>();
            }
...
HTTPMessage node_alert_count(HTTPMessage request, std::shared_ptr<pqxx::connection> connection)
{
    ...
    try
    {
        ...
        int count = 0,total_records;
            ...
            std::string query = "select n.*,count(e.*) from nodes n left join events e on n.host_identifier=e.host_identifier group by n.id,n.config_id,n.host_identifier;";
...
HTTPMessage mitre_analysis(HTTPMessage request, std::shared_ptr<pqxx::connection> connection)
{
    ...
    try
    {
        ...
        for (pqxx::result::const_iterator c = result.begin(); c != result.end(); ++c)
        {
            mitre_analysis[c["technique"].as<std::string>()]=c["count"].as<int>();
        }
        if (result.size() <= 0)
        {
            response.status = boost::beast::http::status::ok;
