...
nlohmann::json SerializeMessage(const BlockingResponse& message)
{
    ...
    payload["event"]["process_entry"]["command_line"] = message.event.entry.command_line;
    payload["event"]["process_entry"]["cwd"] = message.event.entry.current_working_directory;
